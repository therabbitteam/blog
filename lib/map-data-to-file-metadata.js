...
var converter = require('vietnamese-unicode-toolkit')
  ...
  return function (files, metalsmith, done) {
    ...
    for(var file in files) {
      ...
      if (_.has(fileObject, 'prismic') && fileObject.layout === 'blog-post.html') {
         ...
         * Normalize for unicode compound
         */
        files[file].tags = normalize(post.tags)
        /*
         ...
         * Map author
         */
        if (_.has(post.data.author, 'json')) {
          files[file].author = post.data.author.json.slug
        }
      }
    }
